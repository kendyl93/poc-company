...
These are hard review rules for this repository. If implementation conflicts with a rule below, change the implementation.

## Code

- Use TypeScript for all application code.
- Do not use `any` unless there is a documented, unavoidable reason.
- Prefer explicit types at module boundaries, especially for Payload data, block props, shared utilities, and public function inputs or outputs.
...
- Avoid patterns like `HeroClientA` or `HeroClientB` unless the implementation is truly one-off.
...
- A page should be modeled as structured content, typically with fields such as `title`, `slug`, `seo`, and `layout[]`.
- `layout[]` is the source of truth for page composition.
- Reusable page construction must happen through block-based rendering, not through one-off page implementations by default.
- The rendering flow must remain: Payload content -> typed page data -> `BlockRenderer` -> section component -> UI primitives.
- Additions to the page-building system must be done in a coordinated way: update the Payload block schema, update the React section component, register the block in `BlockRenderer`, and add tests.
...
- Treat `packages/theme` as the place for shared visual rules: colors, typography, spacing, radius, and button variants.
- Treat `packages/lib` as the place for cross-cutting utilities such as SEO helpers, form helpers, analytics helpers, and data normalization.
...
- Every new block type must be covered by tests for schema usage, renderer mapping, and rendering with representative props.
- `BlockRenderer` must have tests covering known block rendering, unknown block handling, and block order preservation.
...
- Test user-visible outcomes: rendered content, CTA presence, navigation behavior, fallback states, and form submission flow.
