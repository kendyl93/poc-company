...
function readJson<T>(path: string): T {
  return JSON.parse(readFileSync(new URL(path, repoRoot), "utf8")) as T;
}

describe("shared package scaffolding", () => {
  const packageNames = ["ui", "sections", "theme", "cms", "lib"];
  ...
  it("creates workspace packages with manifests and entrypoints", () => {
    for (const name of packageNames) {
      ...
      expect(existsSync(new URL(packageRoot, repoRoot))).toBe(true);
      expect(packageJson.name).toBe(`@poc-company/${name}`);
      expect(packageJson.scripts).toMatchObject({
        typecheck: expect.any(String),
        test: expect.any(String),
      });
      expect(existsSync(new URL(`${packageRoot}src/index.ts`, repoRoot))).toBe(
        true,
      );
    }
  });
